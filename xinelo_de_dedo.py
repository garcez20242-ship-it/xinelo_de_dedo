...
# --- 1. CONFIGURAÇÃO ---
st.set_page_config(page_title="Gestão Master v8.7", layout="wide", page_icon="🩴")
...
def carregar_banco_completo():
    config_abas = {
        ...
        "Lembretes": ["Data", "Nome", "Vencimento", "Valor", "Categoria"] # Adicionada Categoria
    }
    resultado = {}
...
# --- 5. CABEÇALHO DA PÁGINA INICIAL ---
st.title("🩴 Sistema de Gestão Master")
st.markdown(f"**Última atualização:** {get_data_hora()}")
st.divider()
...
with st.sidebar:
    ...
    # Lembretes Ativos (Categoria: Conta)
    st.subheader("📌 Lembretes Ativos (Contas)")
    contas = df_lem[df_lem['Categoria'].astype(str).str.lower() == 'conta']
    if not contas.empty:
        for _, r in contas.iterrows():
            if str(r['Nome']).strip():
                st.warning(f"**{r['Nome']}**\n📅 {r['Vencimento']}\n💰 R$ {r['Valor']}")
    else: st.caption("Nenhuma conta agendada.")

    st.divider()
    
    # Pendências de Cliente (Categoria: Cliente)
    ...
    if not pends_cli.empty:
        for _, r in pends_cli.iterrows():
            if str(r['Nome']).strip():
                st.error(f"**{r['Nome']}**\n💰 R$ {r['Valor']}\nVencto: {r['Vencimento']}")
    else: st.caption("Nenhuma pendência registrada.")

    st.divider()
    
    with st.expander("🚨 Alerta de Estoque (< 5)"):
        alertas = []
...
with tabs[3]: # HISTÓRICO
    ...
    # Removido filtro agressivo: agora mostra tudo que tiver Data ou Cliente
    if not df_ped.empty:
        df_h = df_ped.dropna(subset=['Data', 'Cliente'], how='all')
        for idx, r in df_h.iloc[::-1].iterrows():
            with st.container(border=True):
                ...
                c_h1.caption(f"Conteúdo: {r['Resumo']}")
...
with tabs[4]: # LEMBRETES
    st.subheader("📅 Cadastro de Lembrete / Pendência")
    with st.form("f_lem_v87"):
        col_l1, col_l2 = st.columns(2)
        l_cat = col_l1.selectbox("Categoria", ["Conta", "Cliente"])
        l_nom = col_l2.text_input("Nome (do Cliente ou da Conta)")
        l_val = col_l1.number_input("Valor R$", min_value=0.0)
        l_ven = col_l2.text_input("Vencimento (Ex: 15/04)")
