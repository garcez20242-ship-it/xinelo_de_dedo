...
ID_PASTA_DRIVE = "COLOQUE_AQUI_O_ID_DA_SUA_PASTA_DO_DRIVE" # O ID fica no final da URL da pasta no Drive

# --- CONFIGURAÇÃO DA CREDENCIAL DE SERVIÇO (DRIVE API) ---
# O Streamlit costuma guardar as credenciais em st.secrets["connections"]["gsheets"]
...
def upload_para_drive(file):
    ...
    # Define permissão de leitura pública para que a imagem apareça no App
...
# --- INTERFACE ---
st.title("👡 Sistema Comercial - Auto-Upload Drive")
...
if conn is not None:
    ...
    with tab1:
                ...
                with st.expander(f"👟 {row['Modelo']}"):
                    ...
                    if modo_edicao:
                        ...
                        if cb1.button("Salvar ✅", key=f"sv_{idx}"):
                            ...
                            st.rerun()
                        with cb2:
                            if st.checkbox("Confirmar?", key=f"chk_m_{idx}"):
                                if st.button("APAGAR", key=f"del_m_{idx}"):
        ...
        with st.form("entrada"):
            ...
            mod_e = c1.selectbox("Modelo", df_estoque['Modelo'].unique())
            ...
            if st.form_submit_button("Registrar Entrada"):
                ...
                df_estoque.at[idx_e, tam_e] = int(pd.to_numeric(df_estoque.at[idx_e, tam_e], errors='coerce') or 0) + qtd_e
                ...
                atualizar_planilha("Estoque", df_estoque); atualizar_planilha("Pedidos", df_pedidos); st.rerun()
    ...
    with tab2:
        ...
        if not df_clientes.empty:
            ...
            with c1:
                ...
                if st.button("Adicionar"): st.session_state.carrinho.append({"Modelo": v_mod, "Tamanho": v_tam, "Qtd": v_qtd})
            with c2:
    ...
    with tab3:
        for idx, row in df_clientes.iterrows():
            with st.expander(f"👤 {row['Nome']}"):
                ...
                col_t.write(f"Loja: {row.get('Loja','')} | Tel: {row.get('Telefone','')}")
                if col_d.checkbox("Confirmar?", key=f"c_cl_{idx}"):
                    if col_d.button("Apagar", key=f"d_cl_{idx}"):
    ...
    with tab4:
        for idx in reversed(df_pedidos.index):
            ...
            if c_a.checkbox("?", key=f"c_h_{idx}"):
                if c_a.button("X", key=f"d_h_{idx}"):
        ...
        if tipo == "Modelo":
            with st.form("f_mod"):
                ...
                if st.form_submit_button("Cadastrar"):
                    ...
                    ni = {"Modelo": m_n, "Imagem": img_url}; ni.update(q_d)
